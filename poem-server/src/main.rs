...
#[handler]
fn log(req: Json<serde_json::Value>) -> Json<serde_json::Value> {
    println!("{:?}", req.0);
...
async fn main() -> Result<(), std::io::Error> {
    ...
    let serve_dir_path = std::env::args().nth(1).expect("must provide path to directory of static files to serve");
    let app = Route::new().nest(
        "/",
        StaticFilesEndpoint::new(serve_dir_path).index_file("index.html"),
    ).at("/log", post(log));
